...
`/totem` 
permission -> `totem.command` 
description -> The base totem command.
`/totem give <totem-item>` 
permission -> `totem.command.give` 
description -> gives the user a totem.
`/pineapple-totem` 
permission -> `pineapple-lib.command` 
description -> My libraries internal commands for when things go wrong.
`/pineapple-totem debug` 
permission -> `pineapple-lib.command.debug` 
description -> provides subcommands with debug information that could be useful.
`/pineapple-totem debug version` 
perimssion -> `pineapple-lib.command.debug.version` 
description -> provides version information for debugging purposes

## Configuration

Unlike most other plugins totem does a lot of its configuration in JSON. Json stands for JavaScript Object Notation. Writing JSON is an easier alternative that leaves you with clear linting when things go wrong. However, most generic text editors may not notify you of these errors. To ensure your JSON is valid before starting your server you can use [JsonLint](https://jsonlint.com/) which will tell you if your JSON has any errors. Alternatively you can use VSCode, Brackets, Sublime and even Notepad++ can allow for seemless JSON editing.
...
:::warning
The duration is in TICKS not seconds or miliseconds. there are 20 ticks in a second!!!
:::
:::info
While there is a defuault duration of 0 it is reommend you always fill out this field
:::
...
:::warning
The "amplifier" starts at 0 instead of 1. E.g. a potoin with amplifier of 0 is level 1 and a potion with amplifier of 1 is 2 and so on
:::
...
:::danger
This feature was not rigerously tested if unexpected issues occur while using please contact support!
:::
...
:::info
Standard effects are applied on top of the potoin effects that you add to these totem settings
:::
...
:::danger
Chaing IDs when totems are already distributed within your playerbase may cause irreversable damage!
:::
...
The result is the id of the totem item. No other items can be accepted. Only totem items will work. You can refer to the [Totem Items](#totem-items--totem-itemsjson) section for more information on totem items
...
Currently Item Types or "Materials" are preferred over item stack delcerations as they are more stable. If you have any questions regarding ItemStack declreations
